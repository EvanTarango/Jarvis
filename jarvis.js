    document.addEventListener("DOMContentLoaded", function() {
      ...
      function processUserInput() {
        ...
        //math solving//
   else if (/^(?:[Ww][Hh][Aa][Tt]'[Ss]?|[Ww][Hh][Aa][Tt]\s+is\s+|[Ww][Hh][Aa][Tt][Ss]?\s+)?[-+*/.\d\s()]+$/.test(userMessage)) {
  try {
    var problem = userMessage.replace(/^(?:[Ww][Hh][Aa][Tt]'[Ss]?|[Ww][Hh][Aa][Tt]\s+is\s+|[Ww][Hh][Aa][Tt][Ss]?\s+)/i, "").trim();
